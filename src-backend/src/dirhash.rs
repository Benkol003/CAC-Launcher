use core::hash;
use std::{fs::File, hash::Hasher, os::windows::fs::MetadataExt, path::Path, sync::Mutex};
...
pub fn build_dir_manifest(base: &Path, manifestPath: &Path) -> Result<(),Error>{
    ...
    while let Some(e) = entries.next() {
        ...
        if(entry.file_type().is_dir()){
            println!("hashing {}",entry.path().as_os_str().to_str().ok_or(anyhow!("failed to convert &OsStr to &Str"))?); //this is excessive...
            let dpbuf =entry.path();
            let dp = dpbuf.as_path();
            manifest.insert(dp.strip_prefix(base)?.to_path_buf().into_os_string().into_string().map_err(|_|anyhow!("failed to convert OsString to String"))?, 
            hash_directory(dp)?.to_string().into()); //json only supports 64bit ints
        }
    }
    ...
    .write(true)
    .truncate(true)   // Open in append mode
    .open(manifestPath)?;
...
pub fn hash_directory(base_path: &Path) -> Result<u128,Error>{
    ...
    let _ = WalkDir::new(base_path).sort(false).into_iter().par_bridge().try_for_each(|e| -> Result<(),Error> {
        let entry = e?;
        let path = entry.path();
        ...
        if ftype.is_file(){
            ...
            //e.g. symlink or something else. TODO later not of any use atm
        }
